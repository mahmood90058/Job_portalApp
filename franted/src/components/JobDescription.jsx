...
const JobDescription = () => {
   ...
   const isIntiallyApplied = singleJob?.applications?.some(application => application.applicant === user?._id) || false;
    ...
    const applyJobHandler = async () => {
        try {
            const res = await axios.get(`${APPLICANT_API_END_POINT}/apply/${jobId}`, {withCredentials:true});
    ...
    <div className='max-w-7xl mx-auto my-18'>
        ...
        <div className='flex items-center justify-between'>
         ...
         <div className='flex items-center gap-2 mt-4'>
            ...
            <Button onClick={isApplied?null:applyJobHandler} disabled={isApplied} className={`rounded-full  ${isApplied? "bg-gray-600 cursor-not-allowed":"bg-purple-500 hover:bg-purple-600 cursor-pointer"}`}>{isApplied? "Already Applied": "Apply Now"}</Button>
